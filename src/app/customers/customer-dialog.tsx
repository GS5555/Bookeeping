...
export function CustomerDialog({ open, onOpenChange, customer, onSuccess }: CustomerDialogProps) {
  ...
  const onSubmit = (data: CustomerFormValues) => {
    const submittedCustomer: Omit<Customer, 'birthday' | 'anniversary'> & { birthday?: string, anniversary?: string, isApproved?: boolean } = {
      ...
      email: data.email || '',
      phone: data.phone || '',
      isApproved: customer?.isApproved !== undefined ? customer.isApproved : true,
    };
  ...
  const dialogDescription = customer ? `Details for: ${customer.name}` : "Fill in the details to create a new customer.";
      ...
      <DialogContent className="sm:max-w-3xl">
        ...
        <Form {...form}>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 max-h-[70vh] overflow-y-auto px-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {isEditing ? (
                <FormField control={control} name="companyName" render={({ field }) => (<FormItem><FormLabel>Company Name</FormLabel><FormControl><Input placeholder="Acme Corp" {...field} /></FormControl><FormMessage /></FormItem>)} />
              ) : <ReadOnlyField label="Company Name" value={getValues('companyName')} />}
              {isEditing ? (
                <FormField control={control} name="gstNumber" render={({ field }) => (<FormItem><FormLabel>GST Number</FormLabel><FormControl><Input placeholder="29ABCDE1234F1Z5" {...field} /></FormControl><FormMessage /></FormItem>)} />
            ...
            <div>
                        ...
                        <Card key={field.id} className="relative">
                          {isEditing && (
                            <CardHeader className="flex flex-row items-center justify-between pb-2">
                                ...
                                {fields.length > 1 && <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-destructive ml-2" onClick={() => remove(index)}><Trash2 className="h-4 w-4" /></Button>}
                            </CardHeader>
                          )}
                          <CardContent className="space-y-4 pt-4">
                            {isEditing ? (
                              <>
                                <FormField control={form.control} name={`addresses.${index}.street`} render={({ field }) => (<FormItem><FormLabel>Street Address</FormLabel><FormControl><Input placeholder="123 Cricket Lane" {...field} /></FormControl><FormMessage /></FormItem>)} />
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <FormField control={form.control} name={`addresses.${index}.city`} render={({ field }) => (<FormItem><FormLabel>City</FormLabel><FormControl><Input placeholder="Bengaluru" {...field} /></FormControl><FormMessage /></FormItem>)} />
                                    <FormField control={form.control} name={`addresses.${index}.state`} render={({ field }) => (<FormItem><FormLabel>State</FormLabel><Select onValueChange={field.onChange} value={field.value || ''}><FormControl><SelectTrigger><SelectValue placeholder="Select state" /></SelectTrigger></FormControl><SelectContent>{indianStates.map(state => (<SelectItem key={state} value={state}>{state}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)} />
                                    <FormField control={form.control} name={`addresses.${index}.zip`} render={({ field }) => (<FormItem><FormLabel>ZIP</FormLabel><FormControl><Input placeholder="560001" {...field} /></FormControl><FormMessage /></FormItem>)} />
                                </div>
                              </>
                ...
                {isEditing && <Button type="button" variant="outline" size="sm" className="mt-4" onClick={() => append({ id: `addr_${Date.now()}`, street: "", city: "", state: "Maharashtra", zip: "", country: "India", isPrimary: false })}><PlusCircle className="mr-2 h-4 w-4" /> Add Address</Button>}
            </div>
            <DialogFooter>
