...
export function SaleDialog({ children, open, onOpenChange, sale, onSuccess }: SaleDialogProps) {
  ...
  const lastItemProductId = watchedItems.length > 0 ? watchedItems[watchedItems.length - 1].productId : "";
  useEffect(() => {
    if (lastItemProductId && open) {
  ...
  const onInvalid = (errors: any) => {
    ...
    toast({
        ...
        description: "The form has errors. Please check the red fields and messages at the bottom.",
        variant: "destructive",
    });
  };

  const dialogTitleText = sale?.id ? "Edit Sale" : "New Sale";
          ...
          <form id="sale-form" onSubmit={form.handleSubmit(handleFormSubmit, onInvalid)} className="space-y-6 max-h-[70vh] overflow-y-auto px-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end pt-4">
                <FormField control={form.control} name="customerId" render={({ field }) => ( 
                  <FormItem>
                    <FormLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-1">
                        Customer Registry <span className="text-destructive">*</span>
                    </FormLabel>
                    <div className="flex items-center gap-2">
                      <Combobox options={customers?.map(c => ({ value: c.id, label: c.name })) || []} value={field.value} onChange={field.onChange} placeholder="Search customers..." searchPlaceholder="Type name..." className="h-10" />
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b pb-2">
                  <FormLabel className="text-lg font-black uppercase tracking-tight">Invoice Line Items</FormLabel>
                  <Button type="button" variant="outline" size="sm" onClick={() => append({ productId: "", brandId: "", handPreference: 'Normal', quantity: 1, unitPrice: 0, hsnCode: '', gstRate: 0, color1: '', color2: '', categoryId: '', subCategoryId: '' })}><PlusCircle className="mr-2 h-4 w-4" /> Add Line Item</Button>
              </div>
              {fields.map((field, index) => {
                  ...
                  const categoryName = categories?.find(c => c.id === watchedItems[index]?.categoryId)?.name || 'N/A';
                  const subCategoryName = subCategories?.find(sc => sc.id === watchedItems[index]?.subCategoryId)?.name || 'N/A';

                  return (
                      <Card key={field.id} className={cn(
                          "border-2 shadow-sm overflow-hidden transition-colors",
                          selectedProdId ? "bg-primary/[0.03] border-primary/20" : "bg-card"
                      )}>
                              ...
                              <div className="flex items-center gap-4">
                                  ...
                                  {selectedProdId && (
                                      <div className="flex gap-2 text-[10px] font-black uppercase">
                                          <span className="text-orange-600 bg-orange-50 px-1.5 py-0.5 rounded border border-orange-200">STOCK: {stock}</span>
                                          <span className="text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded border border-blue-200">TAX: {watchedItems[index].gstRate}%</span>
                                          <span className="text-primary bg-primary/5 px-1.5 py-0.5 rounded border border-primary/20">{categoryName} &gt; {subCategoryName}</span>
                                      </div>
                                  )}
                              </div>
                          ...
                          <CardContent className="p-4">
                              <div className="grid grid-cols-12 gap-3 items-end">
                                      ...
                                      <FormField control={form.control} name={`items.${index}.productId`} render={({ field: f }) => (
                                          <FormItem>
                                              <Popover open={productSearchOpen[index]} onOpenChange={(o) => setProductSearchOpen(prev => ({ ...prev, [index]: o }))}>
                                                  <PopoverTrigger asChild>
                                                      <Button variant="outline" className="w-full justify-between font-normal text-left h-10 border-muted-foreground/50">
                                                          <span className="truncate">{f.value ? allProducts?.find(p => p.id === f.value)?.name : "Search Product..."}</span>
                                                              ...
                                                              <CommandGroup>
                                                                  {allProducts?.map(p => (
                                                                      <CommandItem key={p.id} value={`${p.name} ${p.sku}`} onSelect={() => {
                                                                          const existingIndex = watchedItems.findIndex((item, i) => i !== index && item.productId === p.id);
                                                                          
                                                                          if (existingIndex > -1) {
                                                                              const currentQty = Number(getValues(`items.${index}.quantity`)) || 1;
                                                                              const existingQty = Number(getValues(`items.${existingIndex}.quantity`)) || 0;
                                                                              setValue(`items.${existingIndex}.quantity`, existingQty + currentQty);
                                                                              remove(index);
                                                                              toast({ title: "Item consolidated", description: `Increased quantity for ${p.name}.` });
                                                                          } else {
                                                                      ...
                                                                      }}>{p.name} <span className="ml-auto text-xs opacity-50">{p.sku}</span></CommandItem>
                                                                  ))}
                                                              </CommandGroup>
                                                          </CommandList>
                                                      </Command>
                                  ...
                                  <div className="col-span-6 lg:col-span-2">
                                      <FormField control={form.control} name={`items.${index}.handPreference`} render={({ field: f }) => ( <FormItem><Select onValueChange={f.onChange} value={f.value}><FormControl><SelectTrigger className="h-10 border-muted-foreground/50"><SelectValue /></SelectTrigger></FormControl><SelectContent>{handPreferences?.map(hp => <SelectItem key={hp.id} value={hp.name}>{hp.name}</SelectItem>)}</SelectContent></Select></FormItem> )} />
                                  </div>
                                  <div className="col-span-3 lg:col-span-2">
                                      <FormField control={form.control} name={`items.${index}.quantity`} render={({ field: f }) => ( <FormItem><FormControl><Input type="number" {...f} className="h-10 border-muted-foreground/50" /></FormControl></FormItem> )} />
                                  </div>
                                  <div className="col-span-3 lg:col-span-3">
                                      <FormField control={form.control} name={`items.${index}.unitPrice`} render={({ field: f }) => ( <FormItem><FormControl><Input type="number" {...f} className="h-10 font-black border-muted-foreground/50" /></FormControl></FormItem> )} />
                                  </div>
                              </div>
                          </CardContent>
                      </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-muted/10 p-6 rounded-xl border">
                <div className="space-y-4">
                    <h4 className="font-bold text-xs uppercase tracking-widest text-muted-foreground">Financial Parameters</h4>
                ...
                <div className="space-y-4">
                    <h4 className="font-bold text-xs uppercase tracking-widest text-muted-foreground">Settlement</h4>
            ...
            {Object.keys(form.formState.errors).length > 0 && (
                <div className="p-4 bg-destructive/10 border border-destructive/20 rounded-lg text-destructive text-sm font-medium animate-in fade-in slide-in-from-bottom-2">
                    <p className="flex items-center gap-2 mb-2 font-bold uppercase tracking-tight">
                        <AlertCircle className="h-4 w-4" /> Please rectify the following issues to save:
                    </p>
                    <ul className="list-disc list-inside space-y-1">
                        {Object.entries(form.formState.errors).map(([key, error]) => (
                            <li key={key}>
                                <span className="capitalize">{key.replace(/([A-Z])/g, ' $1')}:</span> {(error as any).message || 'Invalid value'}
                            </li>
                        ))}
                    </ul>
                </div>
            ...
            <div className="rounded-2xl border-2 border-primary/20 p-8 bg-primary/5 shadow-inner space-y-3">
                <div className="flex justify-between text-sm"><span>Subtotal</span><span className="font-bold">₹{totals.subTotal.toLocaleString('en-IN')}</span></div>
                { totals.totalDiscount > 0 && <div className="flex justify-between text-sm text-destructive"><span>Total Discount</span><span className="font-bold">-₹{totals.totalDiscount.toLocaleString('en-IN')}</span></div>}
                { watchedSaleType === 'GST' && (
                ...
                { totals.roundOff !== 0 && <div className="flex justify-between text-[10px] font-bold text-muted-foreground italic"><span>Round Off</span><span>₹{totals.roundOff.toLocaleString('en-IN', {minimumFractionDigits: 2})}</span></div>}
                <div className="flex justify-between items-center pt-4 border-t-2 border-primary/20">
                    <span className="text-2xl font-black tracking-tighter uppercase">Net Payable</span>
                    <span className="text-4xl font-black text-primary tracking-tighter">₹{totals.totalAmount.toLocaleString('en-IN')}</span>
                </div>
            </div>
          </form>
        </Form>
