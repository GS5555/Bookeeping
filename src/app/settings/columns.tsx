...
import { Category, SubCategory, Brand, Color, Courier, Company } from "@/lib/types"

type Item = Category | SubCategory | Brand | Color | Courier | Company;
...
export const columns = (options: {onEdit: (item: Item) => void, onDelete: (itemId: string) => void}): ColumnDef<Item>[] => [
  {
  ...
  },
   {
    accessorKey: "hsnCode",
    header: "HSN Code",
    cell: ({ row }) => (row.getValue("hsnCode") as string) || 'N/A'
  },
   {
    accessorKey: "gstRate",
    header: "GST Rate",
    cell: ({ row }) => {
       const rate = row.getValue("gstRate") as number | undefined;
...
export const subCategoryColumns = (categories: Category[]) => (options: {onEdit: (item: Item) => void, onDelete: (itemId: string) => void}): ColumnDef<SubCategory>[] => [
  ...
  },
  {
    ...
    header: "HSN Code",
    cell: ({ row }) => {
        const subCatHsn = row.getValue("hsnCode") as string | undefined;
        if (subCatHsn) {
            return subCatHsn;
        }
        const parentCategory = categories.find(c => c.id === row.getValue("categoryId"));
        return parentCategory?.hsnCode || 'N/A';
    }
  },
   {
    accessorKey: "gstRate",
    header: "GST Rate",
    cell: ({ row }) => {
       const subCatGst = row.getValue("gstRate") as number | undefined;
       if (subCatGst !== undefined && subCatGst !== null) {
            return `${subCatGst}%`;
       }
       const parentCategory = categories.find(c => c.id === row.getValue("categoryId"));
       if (parentCategory?.gstRate !== undefined && parentCategory?.gstRate !== null) {
           return `${parentCategory.gstRate}%`;
       }
       return 'N/A';
    }
  },
...
export const courierColumns = (options: {onEdit: (item: Courier) => void, onDelete: (itemId: string) => void}): ColumnDef<Courier>[] => [
  ...
  },
  {
    ...
    header: "Tracking URL",
    cell: ({ row }) => (row.getValue("trackingUrl") as string) || 'N/A'
  },
  {
    id: "actions",
    cell: ({ row }) => <ActionsCell item={row.original} onEdit={options.onEdit} onDelete={options.onDelete} />,
