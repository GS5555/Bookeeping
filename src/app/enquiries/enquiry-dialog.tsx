  ...
  useEffect(() => {
    if (open) {
        if(enquiry) {
            ...
            setIsEditing(true);
            reset({ customerId: "", enquiry: "", status: "New", items: [] });
        }
    }
  }, [open, enquiry, reset]);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {isEditing ? (
                    <FormField control={control} name="status" render={({ field }) => (
                        <FormItem><FormLabel className="text-[10px] font-black uppercase tracking-widest">Status <span className="text-destructive">*</span></FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger className="h-10"><SelectValue placeholder="Status" /></SelectTrigger></FormControl><SelectContent>{statuses?.map(s => <SelectItem key={s.id} value={s.name}>{s.name}</SelectItem>)}</SelectContent></Select></FormItem>
                    )}/>
                  ) : <ReadOnlyField label="Status" value={getValues('status')} />}
              </div>
              <FormField control={control} name="enquiry" render={({ field }) => (
                  <FormItem><FormLabel className="text-[10px] font-black uppercase tracking-widest">Details <span className="text-destructive">*</span></FormLabel>{isEditing ? <FormControl><Textarea {...field} className="min-h-24" /></FormControl> : <div className="text-sm p-3 border rounded-md bg-muted whitespace-pre-wrap">{field.value}</div>}</FormItem>
              )}/>
                      ...
                      <Card key={field.id} className="border-2 shadow-sm bg-accent/5 overflow-hidden">
                          ...
                          <CardContent className="p-4 grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
                              ...
                              <div className="sm:col-span-2">
                                  {isEditing ? <FormField control={control} name={`items.${index}.quantity`} render={({ field: f }) => <Input type="number" {...f} className="h-10" />} /> : <ReadOnlyField label="Qty" value={getValues(`items.${index}.quantity`)} />}
                              </div>
                          </CardContent>
                      </Card>
                  ))}
